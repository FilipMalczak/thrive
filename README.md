...
> CircleCI status:
>
> `master` = `0.2.0` [![CircleCI](https://circleci.com/gh/thrive-framework/thrive/tree/master.svg?style=svg)](https://circleci.com/gh/thrive-framework/thrive/tree/master)
>
> Current SNAPSHOT: `0.3.0-SNAPSHOT` [![CircleCI](https://circleci.com/gh/thrive-framework/thrive/tree/0.2.0.svg?style=svg)](https://circleci.com/gh/thrive-framework/thrive/tree/dev)
...
> todo reference service plugin

Then, add a dependency on `thrive-common`:

> todo write down how versioning happens here, what is the current version, etc; for now - `0.3.0-SNAPSHOT` is your friend
...
- `Undecided version, but high prio`
    - try using embedded Kafka and Zookeeper for tests (unbroken dome developed some things like that, check it out),
    optimize them a bit (there is a lot of startup/teardown stuff that can be speedened up)

### Low prio

- stop autoconfiguring everything, take required autoconfigs from properties generated by service plugin
